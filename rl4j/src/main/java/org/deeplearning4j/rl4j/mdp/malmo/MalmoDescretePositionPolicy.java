...
public class MalmoDescretePositionPolicy implements MalmoObservationPolicy {
    ...
    public boolean isObservationConsistant(WorldState world_state, WorldState original_world_state) {
        double[] last_observation = observationSpace.decode(world_state);
        double[] old_observation = observationSpace.decode(original_world_state);

        INDArray newvalues = old_observation == null ? null : Nd4j.create(old_observation);
        INDArray oldvalues = last_observation == null ? null : Nd4j.create(last_observation);
