...
public class AgentListenerList<OBSERVATION extends Observation, ACTION extends Action> {
    protected final List<AgentListener<OBSERVATION,ACTION>> listeners = new ArrayList<>();

    /**
     * Add a listener at the end of the list
     * @param listener The listener to be added
     */
    public void add(AgentListener<OBSERVATION,ACTION> listener) {
     ...
     */
    public boolean notifyBeforeEpisode(SteppingAgent<OBSERVATION,ACTION> agent) {
        for (AgentListener<OBSERVATION,ACTION> listener : listeners) {
     ...
     * @return False if the processing should be stopped
     */
    public boolean notifyBeforeStep(SteppingAgent<OBSERVATION,ACTION> agent, OBSERVATION observation, ACTION action) {
        for (AgentListener<OBSERVATION,ACTION> listener : listeners) {
     ...
     * @return False if the processing should be stopped
     */
    public boolean notifyAfterStep(SteppingAgent<OBSERVATION,ACTION> agent, StepResult stepResult) {
        for (AgentListener<OBSERVATION,ACTION> listener : listeners) {
     ...
     * @param agent The agent that generated the event.
     */
    public void notifyAfterEpisode(SteppingAgent<OBSERVATION,ACTION> agent) {
        for (AgentListener<OBSERVATION,ACTION> listener : listeners) {
