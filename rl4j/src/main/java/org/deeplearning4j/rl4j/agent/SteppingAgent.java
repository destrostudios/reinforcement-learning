...
public class SteppingAgent<OBSERVATION extends Observation, ACTION extends Action>
		implements Agent<OBSERVATION, ACTION> {
	@Getter
	...
	@Getter
	private final Policy<OBSERVATION, ACTION> policy;

	private final TransformProcess<OBSERVATION> transformProcess;

	private final HistoryProcessor<OBSERVATION> historyProcessor;

	protected final AgentListenerList<OBSERVATION, ACTION> listeners;
	 ...
	 */
	public SteppingAgent(@NonNull Environment<ACTION> environment,
			@NonNull TransformProcess<OBSERVATION> transformProcess, @NonNull Policy<OBSERVATION, ACTION> policy,
			HistoryProcessor<OBSERVATION> historyProcessor, @NonNull Configuration configuration, String id) {
		Preconditions.checkArgument(
				configuration.getMaxEpisodeSteps() == null || configuration.getMaxEpisodeSteps() > 0,
				"Configuration: maxEpisodeSteps must be null (no maximum) or greater than 0, got",
				configuration.getMaxEpisodeSteps());
	...
	protected AgentListenerList<OBSERVATION, ACTION> buildListenerList() {
		return new AgentListenerList<OBSERVATION, ACTION>();
	}
	 ...
	 * @param listener
	 */
	public void addListener(AgentListener<OBSERVATION, ACTION> listener) {
	...
	@Override
	public HistoryProcessor<OBSERVATION> getHistoryProcessor() {
	...
	protected void runEpisode() {
		...
		while (canContinue && !environment.isEpisodeFinished()
				&& (maxEpisodeSteps == null || episodeStepCount < maxEpisodeSteps)) {
			performStep();
		}
	...
	protected ACTION getInitialAction() {
		return environment.getActionSpace().getNoOp();
	}
	...
	protected StepResult act(ACTION action) {
		...
		if (historyProcessor != null) {
			historyProcessor.add(observation);
		}
	...
	protected OBSERVATION convertChannelDataToObservation(StepResult stepResult, int episodeStepNumberOfObs) {
		return transformProcess.transform(stepResult.getChannelsData(), episodeStepNumberOfObs,
	...
	public static class Configuration extends Agent.Configuration{
		/**
		 * The maximum number of steps an episode can have before being interrupted. Use
		 * null to have no max.
		 */
